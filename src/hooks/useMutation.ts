...
const useMutation = <T>(mutation: MutationRequest, initialParams = {}) => {
  const [data, setData] = useState<T | null>(
    generateInitialState(mutation.schema) as T,
  );
  ...
  const [isLoading, setIsLoading] = useState<boolean>(
    mutation.initialLoading ?? false,
  );

  const fetchExistingData = async () => {
    if (mutation.existingDataRequest) {
      const response = await axiosClient(
        mutation.existingDataRequest,
        initialParams,
      );
  ...
  const register = (name: keyof T) => ({
    ...
    errorMessage: errors?.[name as string]?.[0],
  ...
  const partialValidation = (fields: string[]) => {
    ...
    if (!result.success) {
      const fieldErrorsTyped: Record<string, string[]> = result.error.flatten()
